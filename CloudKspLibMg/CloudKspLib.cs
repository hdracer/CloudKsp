...
{
    [Guid("FF4B7113-A460-43AD-9901-2AC7B377BBB5")]
    [ComVisible(true)]
    public interface ICloudKsp
    {
        void CreateKey(
            [MarshalAs(UnmanagedType.LPWStr)] String authClientId, 
            [MarshalAs(UnmanagedType.LPWStr)] String authClientSecret,
            [MarshalAs(UnmanagedType.LPWStr)] String vaultAddress,
            [MarshalAs(UnmanagedType.LPWStr)] String keyName,
            [MarshalAs(UnmanagedType.LPWStr)] ref StringBuilder publicModulusB64);
        ...
        void Sign(
            [MarshalAs(UnmanagedType.LPWStr)] String digestB64,
            [MarshalAs(UnmanagedType.LPWStr)] ref StringBuilder signatureB64);

        void Decrypt(
            [MarshalAs(UnmanagedType.LPWStr)] String encryptedB64,
            [MarshalAs(UnmanagedType.LPWStr)] ref StringBuilder decryptedB64);
    }

    [Guid("FF4B7113-A460-43AD-9901-2AC7B377BBB6")]
    [ComVisible(true)]
    public class AzureKspLib : ICloudKsp
    {
        private ClientCredential _clientCredential = null;
        private KeyVaultClient _keyVaultClient = null;
        private KeyBundle _keyBundle = null;

        private async Task<string> _GetAccessToken(
            string authority, 
            string resource, 
            string scope)
        {
        ...
        private KeyVaultClient _Authenticate(
            String authClientId,
            String authClientIdSecret)
        {
            if (null == _keyVaultClient)
            {
                _clientCredential = new ClientCredential(
                    authClientId, authClientIdSecret);
                _keyVaultClient = new KeyVaultClient(
                    new KeyVaultClient.AuthenticationCallback(_GetAccessToken),
        ...
        public void CreateKey(
            ...
            String vaultAddress,
            String keyName,
            ref StringBuilder publicModulusB64)
        {
            //
            // Create an authenticated client
            //

            KeyVaultClient keyVaultClient = _Authenticate(
            ...
            InputValidator iv = new InputValidator(new string[0]);
            KeyBundle kb = iv.GetKeyBundle();
            _keyBundle = keyVaultClient.CreateKeyAsync(
                vaultAddress, 
                keyName, 
                kb.Key.Kty, 
                keyAttributes: kb.Attributes, 
                tags: iv.GetTags()).GetAwaiter().GetResult();

            //
            // Return the public modulus
            //

            publicModulusB64 = new StringBuilder(
                Convert.ToBase64String(_keyBundle.Key.N));
        }
            ...
            [MarshalAs(UnmanagedType.LPWStr)] ref StringBuilder publicModulusB64)
        {
            ...
            InputValidator iv = new InputValidator(new string[0]);
            _keyBundle = keyVaultClient.GetKeyAsync(
                vaultAddress,
            ...
            [MarshalAs(UnmanagedType.LPWStr)] ref StringBuilder signatureB64)
        {
            ...
            byte[] digest = Convert.FromBase64String(digestB64.ToString());

            //
            // Infer hash algorithm from digest size
            //
            ...
            switch (digest.Length)
            {
                ...
                case 64:
                    ...
                    break;
                default:
                    algorithm = JsonWebKeySignatureAlgorithm.RSNULL;
                    break;
            }
            ...
            signatureB64 = new StringBuilder(
                Convert.ToBase64String(signature.Result));
        }
        ...
        public void Decrypt(
            ...
            [MarshalAs(UnmanagedType.LPWStr)] ref StringBuilder decryptedB64)
        {
            byte[] encrypted = Convert.FromBase64String(encryptedB64.ToString());

            //
            // Decrypt
            //

            KeyOperationResult decrypted = _keyVaultClient.DecryptAsync(
                _keyBundle.KeyIdentifier.Identifier, 
                JsonWebKeyEncryptionAlgorithm.RSA15, 
                encrypted).GetAwaiter().GetResult();

            //
            // Return the plaintext
            //

            decryptedB64 = new StringBuilder(
                Convert.ToBase64String(decrypted.Result));
        }
    }
    ...
    [Guid("FF4B7113-A460-43AD-9901-2AC7B377BBB7")]
    class Helpers
    {
    ...
    [Guid("FF4B7113-A460-43AD-9901-2AC7B377BBB8")]
    class InputValidator
    {
        ...
        {
            return new Dictionary<string, string> { { "purpose", "Key Vault operations" }, { "app", "CloudKsp" } };
        }
    ...
    [Guid("FF4B7113-A460-43AD-9901-2AC7B377BBB9")]
